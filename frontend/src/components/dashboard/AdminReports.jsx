    ...
    <div className="space-y-8">
      <div className="glass p-6 rounded-2xl border border-slate-200 shadow-md">
        ...
        {adminReportLoading ? (
          ...
          <div className="p-8 text-center bg-slate-100 rounded-xl text-slate-600 text-xs font-semibold border border-dashed border-slate-200">
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="p-4 bg-slate-500/5 border border-slate-200 rounded-xl">
                <span className="text-xxs uppercase tracking-wider text-slate-600 font-bold">Total Physicians</span>
                <h4 className="text-2xl font-black text-slate-800 mt-1">{adminReportData.data.length}</h4>
              </div>
              <div className="p-4 bg-slate-500/5 border border-slate-200 rounded-xl">
                <span className="text-xxs uppercase tracking-wider text-slate-600 font-bold">Sum appointments</span>
                <h4 className="text-2xl font-black text-slate-800 mt-1">
                  {adminReportData.data.reduce((sum, item) => sum + item.totalAppointments, 0)}
                </h4>
              </div>
              <div className="p-4 bg-slate-500/5 border border-slate-200 rounded-xl">
                <span className="text-xxs uppercase tracking-wider text-slate-600 font-bold">Total Sales ($)</span>
                ...
                <tbody className="divide-y divide-slate-100">
                  {adminReportData.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-500/5 transition-colors">
                      ...
                      <td className="py-3.5 text-slate-700">{item.department}</td>
