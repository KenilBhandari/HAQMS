  ...
  return (
    <div className="glass p-6 rounded-2xl border border-slate-200 shadow-md">
      ...
      {doctorQueue.length === 0 ? (
        <p className="text-center py-6 text-slate-600 text-sm">No checked-in patients in queue today.</p>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {doctorQueue.map((t) => (
            <div
              key={t.id}
              className={`p-5 rounded-2xl border shadow-md relative overflow-hidden flex flex-col justify-between ${t.status === 'CALLING' ? 'border-teal-500 bg-teal-500/10' : 'border-slate-200 bg-slate-500/5'}`}
            >
              ...
              <div className="mt-4">
                ...
                <p className="text-xxs text-slate-600 mt-0.5">Contact: {t.patient.phoneNumber}</p>
              </div>
